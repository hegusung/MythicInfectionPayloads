...
class PathSegmentEntry(object):
    ...
    @classmethod
    def create_for_path(cls, path, entry_type=None):
        entry = cls()
        if not entry_type:
...
def for_file(
    ...
    if target_file.startswith('\\\\'):
        ...
        for i, level in enumerate(levels[1:]):
            ...
            if last:
                segment = PathSegmentEntry.create_for_path(level, entry_type=TYPE_FILE)
            else:
                segment = PathSegmentEntry.create_for_path(level, entry_type=TYPE_FOLDER)
            print(segment)
