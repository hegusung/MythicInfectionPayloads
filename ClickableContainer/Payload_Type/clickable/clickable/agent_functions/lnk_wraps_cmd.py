...
class lnk_wraps_cmd(PayloadType):
    name = "lnk_wraps_cmd"
    file_extension = "lnk"
    author = "@hegusung"
    ...
    async def build(self) -> BuildResponse:
        ...
        try:
            ...
            parts = mslex.split(cmd_payload)
            binary = parts[0]
            args = " ".join([mslex.quote(arg) for arg in parts[1:]])

            if binary.lower() == "powershell.exe":
                binary = "C:\\windows\\system32\\windowspowershell\\v1.0\\powershell.exe"
            elif binary.lower() == "cmd.exe":
